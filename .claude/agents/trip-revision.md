...
4. **Duration change**: the request adds or removes days without a mid-trip cutoff, same start. `delta_days` is positive to extend, negative to shorten. For a positive `delta_days`, if the request names which city gets the extra days, set `target_city` to that name exactly as given, whether or not it already appears in `stays[]`. An explicitly named city is a stated instruction, not a guess, so trust it even when it is new to the trip, note in `reason` whether it is an existing or a new city so the skill knows which branch to run. Only when the request does not name a city at all, and there is more than one city already in the itinerary, set `target_city` to `null` and add a `needs_user_input` entry asking which of the itinerary's existing cities should get the extra days, never guess an unnamed city. `affected_days` is the new days only (for an extension) or the removed days only (for a shortening). New days always get `reverify.destination_research: true`, `logistics: true`, `budget: true`, since they need real venues, not a copy of an existing day, this applies equally to a new-city extension. A shortening only needs `reverify.budget: true` (recompute the total), the other two stay false since no new content is needed. If the shortening removes every remaining day of a city, note that in `reason` so the skill knows to also drop that city's stay and its intercity legs.
