...
## <City 1>
### Must-do
| Name | Area | Why it fits (<likes>) | Crowd tactic | Rating (count) | Price level | Maps link | Source |
|---|---|---|---|---|---|---|---|
### Nice-to-have
...
4. Source column is `tool` for anything returned by search_places. If a call returns `{"error"}`, write the row as "could not verify <query>" with Source `could not verify`. Never fill from memory. Every place result carries `maps_url`, a real Google Maps link, write it in the Maps link column so the traveller can open the actual listing. Write "could not verify" in that column when `maps_url` is missing from the result.
