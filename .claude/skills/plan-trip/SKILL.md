...
2. Every slot copies its crowd tactic and Source from the destinations file. Write the venue name as a Markdown link `[name](maps_url)` using that row's Maps link column, or write the plain name with no link when that column reads "could not verify". For `transit` and `free` slots the crowd tactic is `n/a, not a venue` and there is no link. Schedule each venue in the slot its crowd tactic names: a tactic that says arrive at opening, 06:30, 07:00 or before 09:00 goes in the morning slot, a tactic that names lunch or early afternoon goes in the afternoon slot, an evening tactic goes in the evening slot. If the tactic and the slot cannot agree, pick another venue.
