...
| logistics | sonnet | travel-tools: search_places (hotels), get_walking_route, get_rail_route. Write | brief | `02-logistics.md`: 2 stay areas per city with 2 hotel examples each (name, rating, price level), night split across cities, inter-city rail/transit with duration and fare (seeded minutes and fare for Japan routes, live Google Routes TRANSIT for every other route), day-sequence skeleton grouped by area with walking minutes between anchors |
| budget | sonnet | travel-tools: convert_currency. Read, Write | brief | `03-budget.md`: category split (stay, transport, food, activities, buffer), price bands per category in USD and the destination's local currency (an ISO 4217 code the agent determines from the brief, not hardcoded) at today's rate, 2-3 "if over, cut here" alternatives. Every number labelled estimate or tool-verified |
...
| `get_rail_route(origin_city, destination_city)` | Seeded `data/japan_rail.json` from the official [JR Central smartEX fare table](https://smart-ex.jp/en/product/plan/service/) (ordinary reserved, regular season) for Japan routes, live Google Routes TRANSIT mode for every other route (verified 20-09-2026: Paris to Lyon returns a real SNCF TGV itinerary, EUR 115 fare, 120 min) | n/a, local file for Japan / Google Routes (same key as walking) elsewhere | Seed: `{train, duration_min, fare_jpy_reserved, fare_jpy_hikari, source:"seed", source_url}`. Live: `{line, duration_min, distance_m, fare_amount, fare_currency, source:"tool"}` (fare may be `null`, not every transit agency publishes fare data) |
...
Amendment 19-09-2026, after live probing with the repo key: Google Routes API returns no TRANSIT routes anywhere in Japan (London transit works with the identical request, Tokyo to Kyoto, Tokyo intra-city and lat/lng requests all return `{}`). The original `get_transit_route` tool is replaced by `get_walking_route` (Routes WALK mode, verified: Senso-ji to Ueno Park 1,911 m, 29 min) and `get_rail_route` (seeded from the official JR Central table: Tokyo to Kyoto Nozomi reserved ¥13,970, Hikari ¥13,650). No tool checks intra-city bus or metro transit for any destination, agents give walking time or "could not verify" for intra-city legs, on every destination, not only Japan.

Amendment 20-09-2026, after the France (Paris, Lyon) run and a repo-wide audit for destination-specific hardcoding: `get_rail_route` was only ever checking the Japan seed file, so every inter-city leg outside Japan came back "could not verify" even though Google Routes TRANSIT genuinely works outside Japan (re-verified live: Paris to Lyon returns a real SNCF TGV itinerary with duration and a EUR fare). `get_rail_route` now checks the seed file first (still the right path for Japan, where TRANSIT is broken) and falls through to a live TRANSIT call for every other city pair. The budget agent's rule 1 hardcoded `convert_currency(1, "USD", "JPY")`, forcing every trip's local-currency column to Japanese yen regardless of destination, fixed to determine the destination's ISO 4217 currency from the brief first. Several other agent files and this spec's own tool table carried Japan-only wording (a literal "Fare JPY reserved" table header, "temple"-only examples in destination-research and review's like-matching rule) that would have biased or confused behaviour on a non-Japan trip without being a functional block, generalized throughout. `itinerary.schema.json`'s `intercity[].duration_min` was required non-null, which cannot represent an honest could-not-verify inter-city leg, widened to allow `null`.
