...
|---|---|---|---|---|
| Attractions, food areas, sights, hotels, any city | [Google Places API (New), Text Search](https://developers.google.com/maps/documentation/places/web-service/text-search) | `GOOGLE_MAPS_API_KEY` (repo `.env` uses the name `GOOGLE_MAPS_API`, code accepts both) | 5,000 Text Search Pro calls per month, then $32 per 1,000 ([pricing](https://developers.google.com/maps/billing-and-pricing/pricing)) | Returned Kiyomizu-dera, To-ji, Tenryu-ji for "quiet temples in Kyoto" |
| Walking time between anchors, any city | [Google Routes API, WALK mode](https://developers.google.com/maps/documentation/routes/compute_route_directions) | same key | 10,000 Compute Routes Essentials calls per month, then $5 per 1,000 | Senso-ji to Ueno Park: 1,911 m, 29 min |
| Inter-city rail/transit, any two cities | Seeded file `mcp/travel-tools/data/japan_rail.json`, sourced from the official [JR Central smartEX fare table](https://smart-ex.jp/en/product/plan/service/), for Japan routes (Google Routes TRANSIT is broken there, see below), otherwise live [Google Routes API, TRANSIT mode](https://developers.google.com/maps/documentation/routes/compute_route_directions), same key | none for the seed path, `GOOGLE_MAPS_API_KEY` for the live path | n/a (seed) / same Routes quota as walking (live) | Tokyo to Kyoto (seed), Nozomi reserved ordinary: ¥13,970. Paris to Lyon (live, 20-09-2026): SNCF "Paris - Lyon TGV", 120 min, EUR 115 |
| USD to any currency | [Frankfurter](https://www.frankfurter.dev/) | none | Free, no key | 1 USD = 157.89 JPY on 18-09-2026, 1 USD = 0.8726 EUR on 18-09-2026 |
| Weather for dated trips, any lat/lon | [Open-Meteo](https://open-meteo.com/en/docs) | none | Free, no key | Kyoto forecast returned |

### Why Google Routes transit mode needed a Japan-specific fallback

Routes API TRANSIT mode returns an empty response for every request inside Japan. Tested on 19-09-2026 with the repo key: London King's Cross to Paddington returned a Hammersmith & City Line route with the identical request shape, while Tokyo Station to Kyoto Station, Asakusa to Shibuya, and a lat/lng pair in Tokyo all returned `{}`. Drive and walk modes work in Japan. Google's coverage table is JS-rendered so the Japan row could not be quoted, but a Google developer forum thread reports `TRANSIT` missing from `available_travel_modes` in Japan ([discuss.google.dev](https://discuss.google.dev/t/directions-api-transit-mode-returns-zero-results/378267)). **This is a Japan-specific gap, not a general one.** Re-verified 20-09-2026: Paris to Lyon TRANSIT returned a real SNCF TGV itinerary with duration and a EUR fare. So `get_rail_route` checks the Japan seed file first, then falls through to a live TRANSIT call for every other city pair, and only says "could not verify" when both paths come up empty. No tool in this system checks intra-city bus or metro transit, for any destination, only walking, which is a separate and permanent scope limit, not a Japan-only one.
