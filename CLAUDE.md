...
4. **Facts.** `mcp/travel-tools/server.py` exposes five tools: `search_places` (Google Places Text Search, any city), `get_walking_route` (Google Routes, WALK mode, any city), `get_rail_route` (seeded `data/japan_rail.json` from the official JR Central fare table for Japan routes, live Google Routes TRANSIT for every other route, since TRANSIT reliably returns no itinerary inside Japan but does work elsewhere), `convert_currency` (Frankfurter, any ISO 4217 pair), `get_weather` (Open-Meteo, any lat/lon). No tool in this system checks intra-city bus or metro transit for any destination, only walking, so agents never state a metro or bus time for an intra-city leg. Every response is cached 24h in `mcp/travel-tools/.cache/`. Every tool returns `{"error": ...}` instead of raising.
